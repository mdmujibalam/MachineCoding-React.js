...
const CommentNode = ({ node, addComment, deleteComment }) => {
  ...
  return (
    <div>
      <div className="comment-node" key={node.id} ref={rowRef}>
        {node.label}

        <span className="review-container">
          <span className="btn-container" onClick={() => setIsReply(true)}>
            <button className="btn-node">Reply</button>
          </span>
          <span
